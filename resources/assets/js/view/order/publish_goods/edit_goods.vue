<template>
    <el-tabs v-model="activeName">
        ...
        <el-tab-pane label="套餐设置" name="second" :lazy="lazy">
            <setMeal></setMeal>
        </el-tab-pane>
        <el-tab-pane label="产品尺码" name="third" :lazy="lazy">
            <goodsSize></goodsSize>
        </el-tab-pane>
        <el-tab-pane label="推广链接" name="fourth" :lazy="lazy">
            <Extension></Extension>
        </el-tab-pane>
    </el-tabs>
</template>
<script>
    ...
    import setMeal from './set_meal/list';
    import goodsSize from './goods_size/list';
    import Extension from './Extension';
    import {goodsTemplate, goodsIdGet, goodsIdUpdate} from '@/api/goods';

    export default {
        components: {
            upload, VueUeditorWrap, Table, setMeal,goodsSize,Extension
        },
        data() {
            return {
                lazy: true,
        ...
        methods: {
            ...
            goodsAdd() {
                goodsIdUpdate(this.$route.params.id, this.goods).then((response)=>{
        ...
        created() {
            if (this.$route.params.sub !=  undefined) { //跳转到子页
                this.activeName = this.$route.params.sub;
            }
            goodsIdGet(this.$route.params.id).then((response)=>{
